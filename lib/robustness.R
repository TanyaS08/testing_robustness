...
# "ext_order" is a vector of species names specifying the order of extinction (optional)
...
robustness_gradient <- function(graph, spread=seq(from=1, to=99, by=5), ext_order = NULL) {
  ...
  for (percent_loss in spread) {
    ...
    # If an extinction order is provided, use it. Otherwise, initialize a random pool.
    if (!is.null(ext_order)) {
      ext_pool <- ext_order
    } else {
      ext_pool <- unique(pfw_el[, c("resource")])
      ext_pool <- setdiff(ext_pool, primprods)
    }
    
    # Reset the index for the extinction order vector
    ext_index <- 1

    while (pfw_val1 >= (percent_loss / 100) && ext_index <= length(ext_pool)) {

      # name of i (primary extinction)
      if (!is.null(ext_order)) {
          tax_nam <- ext_pool[ext_index]
      } else {
          tax_nam <- sample(ext_pool, 1)
          # Update the extinction pool for the next step (random removal)
          ext_pool <- setdiff(ext_pool, tax_nam)
      }
      ...
      # Increment index for ordered extinction
      if (!is.null(ext_order)) {
